...
class MistralTranslate(TranslateProvider):
    ...
    def translate(self, segments: List[Segment]) -> List[Segment]:
        ...
        if not api_key:
            logger.error("[Translate] Mistral API Key is missing. Cannot proceed.")
        ...
        try:
            return with_retry(run_api_call, self.retry_config, "MistralTranslate")
        except ImportError:
            logger.error("[Translate] 'requests' library not found.")
            raise RuntimeError("Fatal pipeline error")
        except Exception as e:
            logger.error(f"[Translate] Failed Mistral translation: {e}.")
            raise RuntimeError("Fatal pipeline error")
