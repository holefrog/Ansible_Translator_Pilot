...
function handleState(data) {
    if (data.status === 'error') {
        ...
        document.getElementById('progress-msg').innerHTML = `<span style="color:#dc2626;font-weight:bold;">⚠️ ${data.error || data.message || 'Unknown error occurred.'}</span>`;
        hasFinished = true;
    ...
    if (data.status === 'running') {
        ...
        // Hide spinner area, keep progress bar visible for reference
        document.getElementById('spinner-area').style.display = 'none';
